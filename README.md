...
1. Download the repository ZIP from GitHub or clone
   `https://github.com/blazjerman/PLC.git`.
2. Rename the extracted directory to `PL460Arduino` if necessary, then place it
   in the Arduino sketchbook `libraries` directory. The resulting path should
   end in `libraries/PL460Arduino`.
3. Restart Arduino IDE.
4. Select the ESP32-S3 board and open an example from
   **File > Examples > PL460Arduino**.

The repository root is the Arduino library root. It can also be installed with
Arduino IDE's **Sketch > Include Library > Add .ZIP Library** command.
...
### Included firmware and tested version

The generated firmware headers are intentionally included. Microchip's license
permits source and binary redistribution subject to its conditions, and keeping
the images here makes the Arduino library usable without installing MPLAB
Harmony. The full Harmony/G3 SDK is not included because it is unnecessary for
building the library and contains additional components with their own terms.

The included headers correspond byte-for-byte to the `.bin` files in
Microchip's official `smartenergy` repository at commit
[`91b27a76a301a39b9d04fb513ac7ca8acd047ab2`](https://github.com/Microchip-MPLAB-Harmony/smartenergy/commit/91b27a76a301a39b9d04fb513ac7ca8acd047ab2),
dated 2026-07-01. **This is the firmware revision used for the successful
ESP32-S3 and two-board PL460-EK rev5 G3 CENELEC-A tests.** Its hashes are listed
in `NOTICE.md`.

All firmware and Microchip-derived portions remain subject to
`LICENSE-MICROCHIP.md` and `LICENSE-MICROCHIP-APPS.md`, including the requirement
that they be used solely with or in combination with a device manufactured by
or for Microchip.

### Downloading the firmware from Microchip

Clone the official repository and check out the tested revision:

```sh
git clone https://github.com/Microchip-MPLAB-Harmony/smartenergy.git
cd smartenergy
git checkout 91b27a76a301a39b9d04fb513ac7ca8acd047ab2
```

The PHY images are under:

```text
driver/plcPhy/src/bin/
```

The G3 MAC-RT images are under:

```text
driver/g3MacRt/src/bin/
```

Microchip also publishes the stack and examples through its
[MPLAB Harmony Smart Energy repositories](https://github.com/Microchip-MPLAB-Harmony).

### Updating to a newer firmware revision

Newer upstream firmware may change behavior, mailbox compatibility, coupling
requirements or memory use. Treat any revision other than the commit above as
untested until all boot, mailbox, single-board and two-board tests pass.

To try an update:

1. Pull or clone the latest official `smartenergy` repository.
2. Record the exact upstream commit with `git rev-parse HEAD`.
3. Select the matching `.bin` from `driver/plcPhy/src/bin` or
   `driver/g3MacRt/src/bin`.
4. Regenerate the corresponding header from this repository root:

```sh
python3 tools/generate_firmware_headers.py \
  /path/to/smartenergy/driver/plcPhy/src/bin/PLC_PHY_G3_CENA.bin \
  src/firmware/PLC_PHY_G3_CENA.h
```

5. Update the commit, date and SHA-256 table in `NOTICE.md`.
6. Compile every example, run `SingleBoardSelfTest` and
   `AllFirmwareBootTest`, then repeat `G3HelloWorld` with two boards.

Do not replace a tested image merely because a newer file exists. First check
Microchip's release notes, supported PL360/PL460 revision, mailbox ABI, band and
coupling configuration. Keep the existing headers when reproducibility is more
important than testing a new upstream release.
...
## Publishing and Arduino Library Manager

This repository already uses the correct standalone Arduino-library layout:
`library.properties`, source, examples, notices and licenses are at the root.
The original `G3_v2.1.0` development SDK, local history and build output are
deliberately excluded.

Before submitting a release to Arduino Library Manager:

1. Keep all license files and `NOTICE.md` in every source archive.
2. Run Arduino Lint and compile every example.
3. Create a semantic version tag matching `library.properties`, initially
   `0.1.0`.
4. Do not market the project as Microchip-approved or as a certified G3 stack.

Arduino Library Manager indexes tagged releases and requires
